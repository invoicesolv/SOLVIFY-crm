...
export function LeadsList({ workspaceId }: { workspaceId: string }) {
  ...
  return (
    <Card className="bg-neutral-900/50 backdrop-blur-sm border-neutral-800 p-6">
      <div className="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold text-white mb-1">Recent Leads</h2>
          <p className="text-sm text-neutral-400">Manage and qualify your potential clients</p>
        </div>
        ...
        <div className="flex w-full sm:w-auto gap-2">
          <div className="relative flex-1 sm:w-64">
            ...
            <Input 
              ...
              className="pl-9 bg-neutral-800 border-neutral-700 text-white w-full"
            />
          </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 border border-dashed border-neutral-700 rounded-lg">
          <Grid className="h-12 w-12 mx-auto text-neutral-500 mb-4" />
          ...
          {filteredLeads.map(lead => {
            ...
            return (
              <Link key={lead.id} href={`/leads/${lead.id}`}>
                <div className="group bg-neutral-800 hover:bg-neutral-750 border border-neutral-700 hover:border-neutral-600 rounded-lg p-5 transition-all duration-200 h-full">
                  <div className="flex items-start justify-between mb-4">
                    <div className={`p-3 ${isRecent ? 'bg-green-600/20' : 'bg-neutral-700/30'} rounded-lg`}>
                      <Grid className={`h-5 w-5 ${isRecent ? 'text-green-400' : 'text-neutral-400'}`} />
                    </div>
                  ...
                  <h3 className="text-xl font-bold text-white mb-2 truncate" title={lead.lead_name}>
                    {lead.lead_name}
                  </h3>
                  
                  <p className="text-base text-white mb-4 truncate" title={lead.company}>
                    {lead.company}
                  </p>
                  ...
                  <div className="flex flex-wrap gap-2 pt-3 border-t border-neutral-700">
                    ...
                    <Badge className="bg-neutral-600/20 text-neutral-400">
