...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      ...
      return NextResponse.redirect(new URL(`${process.env.NEXT_PUBLIC_SITE_URL}/settings?error=tiktok_auth_failed`));
    }
    ...
    try {
      ...
      console.log('TikTok OAuth: Looking up workspace for user:', user.id);
      ...
      if (!allWorkspaces || allWorkspaces.length === 0) {
        console.error('TikTok OAuth: No workspace found for user:', user.id);
      ...
      return NextResponse.redirect(new URL(`${process.env.NEXT_PUBLIC_SITE_URL}/settings?success=tiktok_connected`));
    } catch (error) {
